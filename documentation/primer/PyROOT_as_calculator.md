...
Note the probably unexpecte form of the quit command: `.q`

Otherwise not bad. You can see that ROOT offers you the possibility not only to
type in `C++` statements, but also advanced mathematical functions,
which live in the `TMath` namespace.

## PyROOT as calculator ##

Onviously, there is an interactive Python shell, completely
independent of ROOT. Launch with the command
...
Leaving the Python shell works with <CTRL-D> (be careful) or
`exit()`. From now on we assume you know how to start the Python
shell, import necessary modules and exit.

Now let's do something more elaborated (it's actually pure Python, no
dependence on ROOT). A numerical example with the well known
geometrical series:
